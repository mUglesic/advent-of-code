...
def find_path(node, visited):
    ...
    if (node in visited and not node.big and double_visited(visited)) or (node in visited and node == "start"):
        return 0
...
def double_visited(visited):

    for i, node1 in enumerate(visited):

        if node1.big: continue
        
        for j, node2 in enumerate(visited):

            if node2.big: continue

            if i != j and node1 == node2:
                return True
